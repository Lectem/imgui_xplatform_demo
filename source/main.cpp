...
int main(int, char**)
{
    ...
    g_window = SDL_CreateWindow("ImGui SDL2+OpenGLES+Emscripten example", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1280,720, SDL_WINDOW_OPENGL|SDL_WINDOW_RESIZABLE);
    ...
    while (!g_done)
    {
        static int previousTick = SDL_GetTicks();
        static const int SCREEN_FPS = 60;
        static const int SCREEN_TICK_PER_FRAME = 1000 / SCREEN_FPS;

        main_loop();

        int frameTicks = SDL_GetTicks() - previousTick;
        if( frameTicks < SCREEN_TICK_PER_FRAME )
        {
            static int cnt = 0;
            if(!(cnt++ % 60)) printf("ticks to wait %d\n",frameTicks);
            //Wait remaining time
